...
df_celltypeprop[, 1:6]=t(apply(df_celltypeprop[, 1:6], 1, function(x){x/sum(x)}))
...
plot_stackbarchartcelloverview=ggplot(aes(y=value, x=primarypm, fill=variable), data=df_celltypeprop_melt) +
  ...
  facet_wrap(~roi_type) +
  labs(y="Proportion of cells", x="", fill=paste0("n=", formatC(nrow(subset(l_lunaphore_crcpm$df_ref_percell, !is.na(l_lunaphore_crcpm$df_ref_percell$celltype))), format="d", big.mark=",")))  +
  scale_fill_manual(values=c(RColorBrewer::brewer.pal(6, "Dark2")))

pdf(paste(Sys.Date(),paste0("_lunaphore_plot_stackbarchartcelloverview.pdf")), height=3.25, width=6)
