...
# CMS breakdown for WES samples

```{r}

df_wes=subset(df_ref_pm_persample, df_ref_pm_persample$wes=="wes" & !is.na(df_ref_pm_persample$cms_subtype))

df_wes_cms=data.frame((table(df_wes$cms_subtype)))
colnames(df_wes_cms)=c("cms_subtype", "value")
df_wes_cms$label=paste0(df_wes_cms$cms_subtype, "\n(", round(df_wes_cms$prop, 1), "%)")

df_wes_cms <- df_wes_cms %>% 
  arrange(desc(cms_subtype)) %>%
  mutate(prop = value / sum(df_wes_cms$value) *100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop )

# Basic piechart
ggplot(df_wes_cms, aes(x="", y=prop, fill=cms_subtype)) +
